...
func createCertificate() ([]byte, []byte, error) {
	webidURI := fmt.Sprintf("https://%s//web/WebID#agent", hostname)
    webidSAN := fmt.Sprintf("DNS:%s", webidURI)  
    webidValue, _ := asn1.Marshal([]asn1.RawValue{ 
        {Tag: 2, Class: 2, Bytes: []byte(webidSAN)},
	})
	...
	certBytes, err := x509.CreateCertificate(
		...
		caPrivKey,
		&cert.URIs, // Add WebID TLS SAN to certificate
	)
	if err != nil {
		return nil, nil, err
	}
